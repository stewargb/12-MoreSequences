...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    expected = [rg.Point(100.0, 50.0), rg.Point(50.0, 100.0), rg.Point(0.0, 50.0), rg.Point(50.0, 0.0) ]
...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    window = rg.RoseWindow(400, 400, 'Test 4: 80 purple dots on a black circle')
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    title = 'PIZZA test 4: 200 slices, thin balck lines on yellow'
    window = rg.RoseWindow(400,400, title)
    circle = rg.Circle(rg.Point(200,200),150)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for slice in slices:
        line = rg.Line(slice,circle.center)
        line.color = color
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    segments = generate_points_on_circle(circle, number_of_segments)
    what = len(segments)
    for k in range(len(segments)):

        line = rg.Line(segments[k], segments[(k+1)%what])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 20, 5, 'silver', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    what = len(segments)
    for k in range(len(segments)):
        line = rg.Line(segments[k], segments[(k + hops_to_next_point) % what])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
